...
def get_weather():
    ...
    try:
        response = requests.get(BASE_URL, params={"q": city, "appid": OPENWEATHER_KEY, "units": "metric"})
        data = response.json()
        if data.get("cod") != 200:
            return jsonify({"error": data.get("message", "City not found")}), 404
        lat, lon = data["coord"]["lat"], data["coord"]["lon"]
        map_url = (f"https://maps.locationiq.com/v3/staticmap?key={LOCATIONIQ_KEY}&center={lat},{lon}&zoom=13&size=600x400&format=png&markers=icon:default|{lat},{lon}")
        weather = {"city": data["name"], "temp": data["main"]["temp"], "humidity": data["main"]["humidity"], "description": data["weather"][0]["description"], "icon": data["weather"][0]["icon"], "lat": lat, "lon": lon, "map_url": map_url}
...
def get_forecast():
    ...
    if not city:
        return jsonify({"error": "Please enter a city name"}), 400
    try:
        response = requests.get(FORECAST_URL, params={"q": city, "appid": OPENWEATHER_KEY, "units": "metric"})
        data = response.json()
        if data.get("cod") != "200":
            return jsonify({"error": data.get("message", "City not found")}), 404
        ...
        for date in sorted(daily_data.keys())[:5]:
            ...
            forecast.append({"date": date, "day": day_of_week, "min_temp": min(temps), "max_temp": max(temps), "description": description, "icon": data['list'][0]['weather'][0]['icon']})
